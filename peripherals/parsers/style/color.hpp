...
#include "../../style/value_types/color.hpp"
...
namespace WikiMarkup { namespace Peripherals { namespace Style { namespace Parser
{
    namespace Internal
    {
        Color toRgb(HslColor const& hsl);
        Color toRgbFromHex(uint32_t hex);
    }

    template GRAMMAR_TEMPLATE_SIGNATURE
    struct color_grammar : qi::grammar <Iterator, Color()>
    {
        using grammar_result = Color;
        ...
        qi::rule <Iterator, grammar_result(), qi::locals <HslColor>> hsla;
        qi::rule <Iterator, grammar_result(), qi::locals <HslColor>> hsl;
